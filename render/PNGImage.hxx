...
//     if ( !buf_ ) return;
  ...
  bool capture_and_write( char *filename ) {
  ...
  void capture( unsigned char* p, unsigned char* d ) {
  ...
  void writeGrayImage( int width, int height, unsigned char* p, char* pngfile ) {
    //   init();
    setRes( width, height );
    setChannel( CHAN_RGB );
    init();
  ...
  void setPixels( unsigned char* p, unsigned char* d ) {
    for ( int i = 0; i < height_; ++i )
      {
	for ( int j = 0; j < width_; ++j )
	  {
...
#if 0
	    // temp
	    if ( (p[np] == 0) && (p[np+1] == 0) && (p[np+2] == 0) )
	      {
		p[np] = p[np+1] = p[np+2] = 255;
	      }
	    // temp
#endif
	    int kchan;
	    ...
	    //           buf_[i][channel_*j+1] = p[np+1];
	    //           buf_[i][channel_*j+2] = p[np+2];
	    if ( (isCaptureDepth_ == true) && (d != NULL) )
	      {
